...
public class FactoryGroupedClasses {
    ...
    public void generateCode(Elements elementUtils, Filer filer) throws IOException {
        TypeElement supperClassElement = elementUtils.getTypeElement(qualifiedClassName);
        String factoryClassName = supperClassElement.getSimpleName() + SUFFIX;

        JavaFileObject jfo = filer.createSourceFile(qualifiedClassName + SUFFIX);
        Writer writer = jfo.openWriter();

        JavaWriter jw = new JavaWriter(writer);
        PackageElement pkg = elementUtils.getPackageOf(supperClassElement);
        if (!pkg.isUnnamed()) {
